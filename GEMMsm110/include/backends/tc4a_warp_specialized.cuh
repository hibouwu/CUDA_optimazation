...
// Stage 4a: raw tc3-style pipeline plus warp-level producer/consumer split.
//
// warp 0: TCGen05 MMA consumer and TMEM allocation
// warp 1: TMA producer
// all warps: completion waits and direct TMEM readback
...
void tc4a_raw_warp_specialized_kernel(
...
class Tc4aRunner {
 public:
  Tc4aRunner(const half* a, const half* b_nk, float* d,
             int m, int n, int k)
      : output_(d), m_(m), n_(n), k_(k) {
    if (m % kTileM != 0 || n % kTileN != 0 || k % kTileK != 0) {
      std::fprintf(stderr,
                   "tc4a raw kernel requires M a multiple of 128, "
    ...
    auto* kernel =
        &tc4a_raw_warp_specialized_kernel<kTileN, kTileK, kStages>;
  ...
  void launch() {
    ...
    check_cuda(cudaGetLastError(), "tc4a_raw_warp_specialized_kernel launch");
  }
