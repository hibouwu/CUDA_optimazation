...
static constexpr unsigned kIdesc = 136316064u;
static constexpr long long kWorkPerInstruction = 1048576LL;
static constexpr int kWarpsPerBlock = 4;
static constexpr int kThreads = 128;
static constexpr char kCaseId[] = "s8_s32_m128n128k32_full_sm_4warp_block";
static constexpr char kPrecision[] = "s8_s32";
static constexpr char kWorkUnit[] = "operation";
static constexpr int kLogicalBits = 8;
static constexpr int kDescriptorBits = 8;
...
__global__ __launch_bounds__(kThreads,1) void benchmark(
  ...
  __syncthreads();
  unsigned long long ad=smem_desc(a,8,4);
  unsigned long long bd=smem_desc(b,8,4);
  ...
  for(int i=0;i<iters;++i){
    ...
    if((threadIdx.x&31)==0){
      asm volatile("{ .reg .pred p; setp.ne.b32 p,%4,0; tcgen05.mma.cta_group::1.kind::i8 [%0],%1,%2,%3,{%5,%6,%7,%8},p; }"::"r"(dst),"l"(ad),"l"(bd),"r"(kIdesc),"r"(enable),"r"(0),"r"(0),"r"(0),"r"(0));
    }
  }
