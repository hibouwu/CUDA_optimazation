...
static constexpr unsigned kIdesc = 138413184u;
static constexpr long long kWorkPerInstruction = 4194304LL;
static constexpr int kWarpsPerBlock = 4;
static constexpr int kThreads = 128;
static constexpr char kCaseId[] = "nvfp4_f32_m128n256k64_full_sm_4warp_block";
static constexpr char kPrecision[] = "nvfp4_f32";
static constexpr char kWorkUnit[] = "flop";
static constexpr int kLogicalBits = 4;
static constexpr int kDescriptorBits = 4;
...
__global__ __launch_bounds__(kThreads,1) void benchmark(
  ...
  __syncthreads();
  unsigned long long ad=smem_desc(a,4,2);
  unsigned long long bd=smem_desc(b,4,2);
  ...
  for(int i=0;i<iters;++i){
    ...
    if((threadIdx.x&31)==0){
      asm volatile("{ .reg .pred p; setp.ne.b32 p,%4,0; tcgen05.mma.cta_group::1.kind::mxf4nvf4.block_scale.block16 [%0],%1,%2,%3,[%5],[%6],p; }"::"r"(dst),"l"(ad),"l"(bd),"r"(kIdesc),"r"(enable),"r"(tsa),"r"(tsb));
    }
  }
