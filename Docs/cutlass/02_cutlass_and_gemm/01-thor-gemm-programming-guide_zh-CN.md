...
Mainloop 按数值格式解释矩阵中的存储值。即使矩阵尺寸相同，低精度、压缩和复数表示也会要求它读取不同的数据。
...
- **Block-Scaled GEMM** 将低精度量化值（Payload）与缩放因子（Scale）分开存储。Payload 是原始元素经过缩放、舍入并转换为低精度格式后保存的数据本体，不包含 Scale。缩放因子的名称取决于它覆盖的元素范围：
  ...
  $$a_{m,k}\approx S_{\mathrm{recon}}^A[\phi_A(m,k)]a_{\mathrm{payload},m,k},\qquad{}
  b_{k,n}\approx S_{\mathrm{recon}}^B[\phi_B(k,n)]b_{\mathrm{payload},k,n}$$

  其中 $A_{\mathrm{payload}}\text{ 与 }B_{\mathrm{payload}}$ 是低精度量化值，$\phi_A\text{ 与 }\phi_B$ 给出各元素使用的 Scale。这里的 $S_{\mathrm{recon}}^A\text{ 与 }S_{\mathrm{recon}}^B$ 表示重建乘数所需的完整缩放；具体格式还可以把它分成张量级和块级因子。后文的 SFA/SFB 保存逐块的 Scale 值，称为 Scale Tensor。Scale Tensor 是保存 Scale 的张量，Tensor Scale 则是整张输入共享的单个缩放因子。第六部分以 NVFP4 说明这些量的格式、生成步骤和使用位置。
...
下面从 Device 向 Atom 阅读，依次说明主机调用、工作分配、局部计算和基本操作。这是组件的包含与组合关系；实际执行顺序由 Kernel 和 Collective 组织。
...
- **Collective 层：协作线程共同完成局部计算的组件。** 它把 TiledMMA、TiledCopy、共享内存布局、流水级和同步机制组织在一起，规定哪些参与者负责加载或计算、数据何时可读，以及缓冲何时可复用。Mainloop 与 Epilogue 是这一层的两类计算组件，前者产生累加结果，后者消费结果并写回输出。
  ...
  使用 Builder 时，调用者提供架构、数据和执行配置，由下面两个编译期入口推导具体实现所需的模板参数，生成对应的 Collective 类型：
  ...
  Builder 在编译期生成 Mainloop 和 Epilogue 类型，Kernel 在运行时调用这两个 Collective。Tiled 和 Atom 已由所选实现配好，调用者可以从 Builder 开始组合 Kernel。五层模型中的 Collective 是执行组件，Builder 是它的类型构造入口。

- **Tiled MMA/Copy 层：规定局部乘加与搬运中的数据分工。** TiledMMA 将 A/B 操作数和累加结果分给参与者，TiledCopy 划分每个参与者负责搬运的源、目标元素。两者都由基本 Atom 与布局组合而成，分区方式遵循 Atom 的操作约束；Collective 再安排这些搬运和乘加何时执行、何时同步。
...
- **Atom 层：基本架构操作及其操作数约束的封装。** 架构操作提供计算或搬运能力，Traits 描述操作需要的 Shape、数据类型和参与者—数据布局，Atom 将它们包装为可供 Tiled 层组合的接口。完整问题由 Kernel 分配，K 维流水线由 Collective 组织；Atom 为这些计算提供基本操作。
  ...
  - [`cute::Copy_Atom<>`](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cute/atom/copy_atom.hpp)：依据搬运操作及其 [`Copy_Traits`](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cute/atom/copy_traits.hpp)，提供基本搬运的调用与源、目标布局接口。切换 TMA、cp.async 等搬运操作时，要按各自的 Traits 核对参与者以及源、目标布局；统一的调用接口保留了这些操作各自的要求。

  **Fragment** 是 MMA 使用的局部操作数或累加结果。[Fragment 构造接口](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cute/atom/mma_atom.hpp#L123-L179)中的 `make_fragment_A/B/C` 接收经过 `partition_A/B/C` 划分的张量，按 Atom 和 Traits 指定的类型构造 A/B 操作数与 C 累加器。A/B 的构造函数根据 Fragment 类型选择沿用已有张量的视图，或创建保存数值的局部张量。因此，判断数据是否位于线程寄存器时，应查看实际生成的张量或视图类型；Fragment 这个名称描述的是操作数的组织方式。内部构造在第三篇继续展开。
...
与运行时 ProblemShape 不同，`CollectiveBuilder` 的 `TileShape_MNK=(T_M,T_N,T_K)` 是编译期参数，表示 M×N×K Collective Tile。$T_M$ 和 $T_N$ 定义一次局部矩阵乘法覆盖的输出行、列范围，$T_K$ 定义 Collective Mainloop 一次 `k_tile` 迭代消费的 K 维宽度。`TileShape_MNK` 固定这个 Kernel 类型使用的局部计算尺寸；某次运行领取哪个 Tile，则由 Scheduler 给出的工作坐标确定。

`ClusterShape_MNK` 描述一个 CTA Cluster 在相应方向上包含多少个 CTA，分量以 CTA 个数为单位，矩阵元素的局部尺寸由 Tile Shape 描述。Cluster 内的 CTA 可以利用所选实现支持的共享数据搬运与同步机制。一个 Cluster 可以包含多组局部计算。一次 MMA 使用单个 CTA 还是一对 CTA，取决于所选 MMA 路径，因此还要结合下面的 1SM/2SM 分工确定每个 CTA 对应的数据范围。
...
两类实现都计算同一项矩阵乘加，并通过 Collective、Kernel 和 Device 接口组合。SIMT 在线程寄存器中执行 FMA，本文的 Tensor Core 路径通过共享内存描述符和 TMEM 执行矩阵 MMA；输入格式、参与范围、累加存储和浮点运算顺序随之不同。第七部分给出 FP32 SIMT 类型，供读者对照这两种实现。

## 4.2 Scenario 到 CUTLASS 模板参数的映射
...
有了这些参数位置，第5部分就可以填入 Dense 的具体配置，依次构造 Collective、Kernel 和主机调用接口。第6部分沿用这条调用过程，说明数值表示和问题集合变化后应怎样调整类型与运行时输入。

# 5. Dense FP16 Tensor Core 完整基线

本章以一次 $D=\alpha AB+\beta C$ 调用说明五层模型的用法。计算组件在编译期确定，问题尺寸和矩阵数据在运行时提供。完整程序 `dense_baseline.cu` 列在本章末尾，文件中的分段注释与本章小节一一对应；下面各段代码共同组成这一个程序，并按构造、输入准备、执行和验证的顺序讲解。类型声明位于函数外，从 Shape/Stride 开始的运行时代码位于 `main()` 中。`check` 的定义为便于讲解放在参考比较之后展示，完整文件中则位于 `main()` 之前。
...
`ElementA/B/C/D` 指定矩阵在内存中的元素类型。`ElementAccumulator` 指定 K 维乘加所形成的累加值类型，`ElementCompute` 指定 Epilogue 执行线性组合时使用的计算类型。本例后二者均为 FP32，分别用于 Mainloop 累加和 Epilogue 计算；输出存储类型由 `ElementD` 单独指定。
...
在本节的 Dense TMA Mainloop 中，一个 Stage 对应一组 A/B Tile 缓冲及其同步状态。加载方等待缓冲可用后写入数据，计算方等待数据就绪后使用它，确认消费完成后才允许复用。`StageCountType` 控制缓冲级数，K Tile 数决定迭代次数；有限个 Stage 会在这些迭代中循环使用。增加 Stage 能保留更多迭代的数据，同时增加共享内存占用，是否更快取决于这些缓冲能否改善搬运与计算的重叠。
...
`StageCountAutoCarveout` 接收要预留的共享内存字节数，Stage 数由 Builder 根据剩余预算推导。所选 Builder 结合每级 A/B 缓冲和同步存储的大小，扣除相应预留后计算可容纳的级数，见 [Dense Builder 的 Stage 推导](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/gemm/collective/builders/sm100_umma_builder.inl#L77-L102)。

Epilogue 的存储需求决定了这两个类型的声明顺序。运行时仍然由 Mainloop 产生累加结果，再由 Epilogue 消费。`KernelScheduleAuto` 与 `EpilogueScheduleAuto` 根据类型、Tile 和架构选择实现，自动 Stage 则计算存储预算。Auto 在编译期根据这些类型和存储约束选择实现；运行时性能测量与配置调优需要另外进行。
...
CuTe 的 `Layout` 对象把 Shape 与 Stride 组合为坐标到线性索引的映射；它可以描述矩阵存储，也可以描述线程与数据的对应关系。前面的 `cutlass::layout::RowMajor` 则是 Builder 的布局标签，用于选择相应的 Stride 类型和实现。标签确定存储方向及对应的 Stride 类型，运行时再填入行间距和批次步长，才能按本次矩阵的实际存储寻址。相关定义见 [CuTe Layout](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/media/docs/cpp/cute/01_layout.md)。
...
- **访问粒度限制合法组合。** 边界处理依据 M/N/K 选择有效坐标，搬运操作还要满足自身的对齐要求。本节紧密排列的 FP16 A/B 使用 16 字节 TMA 访问条件，连续维 K 和 N 分别需要满足 8 个元素的对齐。C/D 的 FP32 访问也要满足各自的对齐要求。换用其他类型或路径时，应先检查该实现对问题尺寸、基址和字节步长的要求，再由 `can_implement` 检查其覆盖的运行时条件。增加分配空间或改大行步长只改变存储安排，问题尺寸仍需满足所选实现的访问条件。
...
这里的 **Workspace** 是调用者在设备全局内存中分配的辅助缓冲，其用途和容量取决于所选 Kernel、本次问题及调度参数；不需要这类空间时，查询结果也可以为零。`SharedStorage` 则描述 Kernel 内部的共享内存需求，用于 Stage 推导和启动配置。两者分别占用设备全局内存与共享内存，作用也不同：Mainloop 的流水级数由共享内存预算确定，增大 Workspace 只会扩大外部辅助缓冲。[Adapter 的初始化实现](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/gemm/device/gemm_universal_adapter.h#L313-L347)分别处理 Workspace 初始化与共享内存需求。
...
修改实例时，先判断改变的是类型还是运行时参数。若 Element、矩阵 Layout Tag、Tile、Cluster、Schedule 和 Fusion 类型保持不变，只更换 M/N/K/L、矩阵地址、可变 Stride 或 alpha/beta，就仍然使用同一个 `GemmKernel` 类型。调用者重新准备相应 Buffer 和 `Arguments`，检查新参数，并按新的 Workspace 需求完成初始化。`run` 读取初始化时保存的内部参数。修改 Host 上的 `arguments` 后，应重新完成初始化，再启动使用新参数的计算。
...
将下面的完整代码复制到一个工作目录，保存为 `dense_baseline.cu`。文件先放置头文件和两节编译期类型构造，再定义 `check`；`main()` 按 Shape/Stride、数据准备、Adapter 调用和参考比较的顺序执行。末尾的迁移说明以注释保留，不增加另一轮计算。正文中的逐参数注释也保留在完整文件中。

完整源代码如下，其中 `iostream` 用于输出验证结果与错误信息：
...
在保存 `dense_baseline.cu` 的目录中执行下面的命令。先将 `/path/to/cutlass` 替换为实际的 CUTLASS 源码目录，该目录应包含 `include/` 和 `tools/util/include/`，版本采用正文固定的 commit。编译使用 CUDA 13.x 及其支持的宿主编译环境：
...
如果已经取得本文所在的仓库，也可以直接进入 `Docs/cutlass/02_cutlass_and_gemm/exemples/`，使用其中的同名源文件执行上述命令。
...
第5部分给出了 Dense 调用的完整过程。本章讨论数值表示、问题集合和阶段依赖变化时，数据与组件需要怎样调整。前三节沿用 Dense 的 Adapter 调用顺序；Attention 则先复用两次 GEMM，再进入专用融合 Kernel。各节的类型或参数代码为根据固定示例整理的节选。五个场景的完整 `.cu` 程序直接列在对应章节末尾，代码注释与前面的讲解相对应。将各节的完整代码保存为指定文件名后，在保存目录中沿用 5.8 的编译命令，并替换输入文件名和输出程序名；各节分别说明其验证范围。
...
- **Attention 改变阶段之间的依赖。** 两次 GEMM 之间需要 Mask、Softmax 和中间结果传递。融合实现由专用 Kernel 组织两次乘法、行归约和跨 Tile 状态更新；普通 GEMM 的 `Fusion` 参数描述单次 GEMM 中的融合操作。

这些选择可以组合。例如，Grouped 或 MoE 的每项 GEMM 都可以使用 Block Scaling：前者规定问题集合，后者规定每项问题的数值表示。下面分别说明它们对 Dense 调用的影响。

## 6.1 Block-Scaled：从 Scale Block 到 SFA/SFB 与 GEMM 调用
...
- **CuTe view 描述数据访问。** 普通 `make_tensor(ptr, layout)` 将已有数据的访问方式与坐标布局组合起来。Engine 包装已有的指针或迭代器，Layout 将坐标映射为偏移，两者共同决定怎样访问数据。求 Scale、执行缩放和生成低精度值则由量化函数中的数值运算完成；创建或传递 view 只建立和传递访问方式。相关对象定义见 [CuTe Tensor 与 Engine](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/media/docs/cpp/cute/03_tensor.md)。
...
本节末尾给出完整程序 `nvfp4_block_scaled.cu`。它沿用下文的类型配置，取 `(M,N,K)=(256,1024,256)`，从原始浮点输入开始执行 CPU 量化，再构造 packed Payload、SFA/SFB 并调用 GEMM；量化误差与 GEMM 计算误差分开检查。
...
- **Tensor Scale：** 整个张量共享一个 FP32 缩放因子，记为 $s_{\mathrm{tensor}}$，用于调整张量的整体范围。

E2M1 中的 E、M 分别表示指数位数和小数位数，再加一个符号位，共用 4 bit 编码一个数值。指数与小数共同决定了这组浮点编码的离散取值，其表示规则与四位整数不同。最大有限幅值为 6，区间内落不到编码取值上的数仍要舍入。UE4M3 是这里的非负 Scale 格式，仍按一个字节保存，Scale 自身也有转换和舍入误差，见[数值格式定义](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/exmy_base.h#L865-L883)。

这种两级结构见 [NVIDIA 对 NVFP4 的说明](https://developer.nvidia.com/blog/introducing-nvfp4-for-efficient-and-accurate-low-precision-inference/)。设原始值为 $x_i$，量化 Payload 为 $x_{\mathrm{payload},i}$，元素 i 所属的块为 $b(i)$，该块的 Scale 为 $s_{\mathrm{block},b(i)}$。文字标签 payload 与 dequant 分别表示量化值和重建值，i 表示元素索引，b(i) 表示所属块编号。它们共同给出重建值：

$$x_{\mathrm{dequant},i}=s_{\mathrm{tensor}}\,s_{\mathrm{block},b(i)}\,x_{\mathrm{payload},i}\approx x_i$$

Payload 与 Block Scale 在不计 $s_{\mathrm{tensor}}$ 时能表示的最大幅值为：

$$6\times448=2688$$

这个数说明 Tensor Scale 的作用：原始张量的范围先由 $s_{\mathrm{tensor}}$ 调整，再由各块的 Scale 把局部数值变换到 E2M1 的范围。下面以非零、有限输入说明按最大幅值选择 Scale 的一种量化过程。具体实现还要处理 Scale 的舍入、下溢与零值。

### 6.1.2 从原始张量生成两级 Scale 与 Payload

令 $\operatorname{amax}(\mathbf{x})=\max_i|x_i|$，$\mathcal I_b$ 表示第 b 个块的 16 个元素。量化可以按四步推导。

1. **求 Tensor Scale。** 用原始张量的最大幅值确定整体缩放：

   $$s_{\mathrm{tensor}}=\frac{\operatorname{amax}(\mathbf{x})}{2688}$$

   $s_{\mathrm{tensor}}$ 用 FP32 保存。该式给出把张量最大幅值缩放到 2688 的目标，计算时仍使用实际保存的 $s_{\mathrm{tensor}}$。

2. **按 Tensor Scale 归一化。** 对每个原始元素计算：

   $$x_{\mathrm{norm},i}=\frac{x_i}{s_{\mathrm{tensor}}}$$

   在理想算术下，归一化后的最大幅值为 2688。这里的 $\mathbf{x}_{\mathrm{norm}}$ 用来说明数值关系，量化实现可以在读取元素时直接完成除法，无须另存整个中间张量。

3. **求每个 Block Scale。** 对归一化后的每个块求最大绝对值，再用 E4M3 保存候选 Scale：

   $$s_{\mathrm{block},b}=\operatorname{cast}_{\mathrm{UE4M3}}\!\left(
   \frac{\max_{i\in\mathcal I_b}|x_{\mathrm{norm},i}|}{6}
   \right)$$

   $\operatorname{cast}_{\mathrm{UE4M3}}$ 表示实际采用的 Scale 转换和舍入规则。后续生成 Payload 时要使用转换后的 $s_{\mathrm{block},b}$，这样计算端乘回的 Scale 才与量化端一致。

4. **生成 E2M1 Payload。** 每个元素再除以所属块的 Scale，并转换为 E2M1：

   $$x_{\mathrm{payload},i}=\operatorname{cast}_{\mathrm{E2M1}}\!\left(
   \frac{x_{\mathrm{norm},i}}{s_{\mathrm{block},b(i)}}
   \right)$$

   $\operatorname{cast}_{\mathrm{E2M1}}$ 按选定规则执行舍入与饱和。保存的结果是量化值 $x_{\mathrm{payload},i}$、逐块的 $s_{\mathrm{block},b}$ 和张量级的 $s_{\mathrm{tensor}}$。

例如，归一化后的一个块最大幅值为 12，则候选 Block Scale 为 2，且 2 可以由 UE4M3 精确表示。块内数值 3.1 除以 2 得到 1.55，按最近值舍入为 E2M1 的 1.5。局部重建值为 $2\times1.5=3$，再乘 $s_{\mathrm{tensor}}$ 才回到原始张量的尺度。两级 Scale 调整了表示范围，E2M1 舍入仍会产生误差。

全零张量可以约定 $s_{\mathrm{tensor}}=1$、各块 Scale=1、Payload 全为 0；非零张量中的全零块也可使用 Scale=1、Payload 全为 0。非零张量的 $s_{\mathrm{tensor}}$ 或非零块的 Scale 若在转换时下溢为零，则需要量化器规定的非零下限或专门处理，避免继续除以零。Scale 舍入还可能使个别缩放值超过 E2M1 的范围，因此即使整体范围已经归一化，Payload 转换仍需明确饱和规则。

### 6.1.3 在 GEMM 中分别放置两级 Scale

A 的每一行、B 的每一列沿 K 划分为长度为 $V=16$ 的块。取 $L=1$、K 为 V 的整数倍，用 b 表示沿 K 的块编号、u 表示块内坐标：

$$k=bV+u,\qquad 0\leq u<V$$

A 的张量级因子记为 $s_{\mathrm{tensor}}^{A}$，逐块 Scale 记为 $s_{\mathrm{block},m,b}^{A}$；B 对应为 $s_{\mathrm{tensor}}^{B}$ 和 $s_{\mathrm{block},n,b}^{B}$。两个操作数的完整重建关系为：

$$a_{\mathrm{dequant},m,bV+u}
=s_{\mathrm{tensor}}^{A}\,s_{\mathrm{block},m,b}^{A}\,a_{\mathrm{payload},m,bV+u},\qquad{}
b_{\mathrm{dequant},bV+u,n}
=s_{\mathrm{tensor}}^{B}\,s_{\mathrm{block},n,b}^{B}\,b_{\mathrm{payload},bV+u,n}$$

SFA/SFB 保存这里的逐块 Scale。A 第 m 行中 $k=16,\ldots,31$ 的元素共享 $s_{\mathrm{block},m,1}^{A}$，下一行的同一 K 区间则有自己的 Scale。独立的逻辑 Scale 数量分别为 $M\times(K/V)$ 和 $N\times(K/V)$；$s_{\mathrm{tensor}}^{A}\text{ 与 }s_{\mathrm{tensor}}^{B}$ 各为一个张量级标量。
...
=\sum_{b\in\mathcal B_r}s_{\mathrm{block},m,b}^{A}s_{\mathrm{block},n,b}^{B}
  \sum_{u=0}^{V-1}a_{\mathrm{payload},m,bV+u}b_{\mathrm{payload},bV+u,n}$$

块编号 b 改变时，$s_{\mathrm{block},m,b}^{A}s_{\mathrm{block},n,b}^{B}$ 也可以改变，因此 Block Scale 需要在 K 维归约中参与计算。Tensor Scale 则对整张 A/B 固定，可以提出归约：

$$d_{\mathrm{ref,quant},m,n}
=\alpha_{\mathrm{original}}\,s_{\mathrm{tensor}}^{A}s_{\mathrm{tensor}}^{B}
  \sum_r\Delta\operatorname{acc}_{m,n}^{(r)}
+\beta C_{m,n}$$

所以 Mainloop 读取量化 A/B 与 SFA/SFB，Epilogue 使用 `alpha_effective = alpha_original * g_A * g_B`，beta 保持原值。公式中的 $s_{\mathrm{tensor}}^{A}$、$s_{\mathrm{tensor}}^{B}$ 在完整程序中分别对应 `qa.tensor_scale`、`qb.tensor_scale`；节选表达式和代码注释中的 `g_A/g_B` 沿用同一含义。若参考计算已经把 $s_{\mathrm{tensor}}^{A}\text{ 与 }s_{\mathrm{tensor}}^{B}$ 乘回 A/B，则应继续使用 `alpha_original`，避免重复计入张量级缩放。
...
### 6.1.4 Scale Layout 由 Kernel 配方和 ProblemShape 共同确定
...
Layout 使用元素坐标定位 Scale：`layout_SFA(m,k,l)` 接收原矩阵的 k，逻辑 Scale 数组的第二个坐标则是量化块编号 b。因此把 $s_{\mathrm{block},m,b}^{A}$ 写入物理 Buffer 时，应使用该块的起点 $k=bV$：
...
以 $V=16$ 为例，`layout_SFA(m,16,0)` 到 `layout_SFA(m,31,0)` 指向同一个 Scale；块内 K 模式的 Stride 为 0，表达了 16 个量化值共享一次缩放。`filter_zeros` 在计算存储量时去掉这种广播重复，但保留物理布局要求的填充。分配容量因此应采用 `count_SFA/count_SFB`，使 Buffer 同时容纳逻辑 Scale 和物理布局要求的填充。

B 在数学中写作 $(k,n)$，而这里的 SFB Layout 按 $(n,k,l)$ 接收坐标：固定 n 后沿 K 选择量化块。A/B Payload 本身也要使用对应的矩阵 Layout 和 FP4 打包存储。

4 bit 描述逻辑 FP4 元素的编码宽度。C++ 中的 `float_e2m1_t` 标量以字节容器保存该编码，普通 `std::vector<float_e2m1_t>` 按标量对象存储，每个元素使用一个字节。供当前 GEMM 使用的紧密打包 Payload 则在一个字节中保存两个 FP4 编码，需要通过支持子字节的容器、引用或迭代器访问，使逻辑元素索引落到正确的 4-bit 位置。固定示例用 [HostTensor 分别管理 Payload 与 Scale 存储](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/examples/72_blackwell_narrow_precision_gemm/72a_blackwell_nvfp4_bf16_gemm.cu#L178-L186)，也可对照 [Array 的子字节特化](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/array_subbyte.h#L46-L78)理解逻辑元素数与实际存储量的区别。
...
- **先检查量化输入的 GEMM 计算。** 保留实际写入的 Payload 和 Scale，用它们重建 A/B，并以高精度完成矩阵乘加。若重建已经包含 $s_{\mathrm{tensor}}^{A}\text{ 与 }s_{\mathrm{tensor}}^{B}$，参考计算使用 `alpha_original`；若只重建局部 Scale 与 Payload 的乘积，则使用 `alpha_effective`。再执行相同的 beta 和输出类型转换，与 Kernel 的 D 按约定容差比较。参考读取逻辑 Scale 坐标，可以帮助发现物理 Layout 重排中的索引错误。
- **再评估量化对原始 GEMM 的影响。** 用未量化的高精度 A/B 计算 $D_{\mathrm{ref,fp}}$，再与 $D_{\mathrm{ref,quant}}$ 比较。前一步已经单独检查了 Kernel 计算，这一步的差异用于衡量输入表示和舍入对整个矩阵乘法的影响。

本节仍采用 Example 72a 的 BF16 输出。如果下一层还需要 NVFP4，就要对 D 再执行一次量化，并为 D 确定自己的 Tensor Scale 和 Block Scale。按上面的动态最大幅值规则计算 Tensor Scale，需要读取整个输出的幅值范围；当输出跨越多个 Tile 时，各 Epilogue 只掌握自己的局部结果。因此，融合输出 requant 前，要先确定输出 Scale 从哪里取得，以及是否需要跨 Tile 归约。输入侧的 Block-Scaled Mainloop 使用已知 Scale；输出侧则要为新生成的 D 确定 Scale。

### 6.1.7 完整程序：NVFP4 Block-Scaled

保存为 `nvfp4_block_scaled.cu`。程序包含 CPU 量化、packed FP4 与 Scale 存储准备、GEMM 调用及独立参考比较。
...
### 6.2.2 用 Shape、地址和 Stride 三类数组描述问题
...
[`GroupProblemShape`](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/gemm/group_array_problem_shape.hpp#L54-L82)为每组保存独立 Shape。Mainloop Builder 的 Layout 参数使用 `LayoutA*`、`LayoutB*`，Epilogue Builder 使用 `LayoutC*`、`LayoutD*`，并分别传入上述 Schedule。这里的星号属于编译期 Layout 类型，用于选择 Pointer-array 实现。运行时传入的是各组矩阵地址和 Stride 数组，Layout 对象本身不参与取地址。
...
由于 Cluster 为动态形状，`hardware_info` 除设备信息外还要填写 `cluster_shape` 和 `cluster_shape_fallback`。固定示例的默认值分别为 `(4,2,1)` 与 `(2,1,1)`，并在[运行参数构造处](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/examples/75_blackwell_grouped_gemm/75_blackwell_grouped_gemm.cu#L570-L584)传入。其他取值也需要满足所选实现和目标设备的 Cluster 要求。调用者在这两个字段中填写本次 Grouped 调用的 Cluster 值；第5部分的静态 `(2,2,1)` Cluster 属于另一个 Dense Kernel 类型。
...
因此，验证时应按问题描述选取边界。每组参考计算读取自己的 $(M_g,N_g,K_g)$ 和输入数据，只比较本组有效的 $M_g\times N_g$ 输出范围。若使用大 Buffer，还需要确认各组起点和步长不会使输出写入相邻组。分配方式和指针数组负责定位数据，每组真实 Shape 负责限定计算与写回范围。

### 6.2.6 完整程序：Grouped GEMM

保存为 `grouped_gemm.cu`。程序完整准备每组 Shape、地址与 Stride 数组，并逐组检查输出。
...
## 6.3 MoE：从 Token 路由得到 Expert GEMM
...
CUTLASS 在本节承担第三步：外层算法先执行 Router 和 Token 聚集，再把准备好的 Expert 数据交给问题描述与 Scheduler；GEMM 输出仍交回外层算法恢复顺序并合并。
...
本节末尾给出完整程序 `moe_expert_gemm.cu`。它取三个 Expert、Token Count 为 `8、17、32`，按下文的最大槽位接口计算，并用固定 Top-1 路由展示输入聚集和输出恢复。路由关系由示例预先给定，不包含 Router 网络、Top-k 加权合并或完整 Expert FFN。

### 6.3.1 从矩阵方向理解 Token Count 为什么位于 N
...
### 6.3.4 分别验证 Expert 计算与路由结果

这两类结果需要分别检查：

- **Expert GEMM 的结果。** 对每个 e 使用实际 $T_e$、聚集后的 Token 和本 Expert 的权重计算参考值，只比较对应的有效输出区域。
- **完整 MoE 的结果。** 检查聚集后的 Token 是否与路由索引一致，Top-k 路由是否按每条有效分支组织输入，routing weight 是否在约定位置应用，以及合并后的结果是否回到原 Token 顺序。这些检查覆盖路由、数据重排和结果合并，逐 Expert 的矩阵比较只覆盖其中的乘法计算。
...
### 6.3.5 完整程序：MoE Expert GEMM

保存为 `moe_expert_gemm.cu`。程序包含固定 Top-1 路由下的输入聚集、Expert GEMM 与输出恢复，不包含 Router 网络或完整 Expert FFN。
...
## 6.4 Attention：从两个 GEMM 到在线 Softmax
...
$$s_{\mathrm{score},i,j}=\frac{\mathbf q_i^T \mathbf k_j}{\sqrt d}+mask_{i,j},\qquad{}
p_{\mathrm{prob},i,j}=\frac{\exp(s_{\mathrm{score},i,j}-m_{\mathrm{row},i})}{\ell_{\mathrm{exp},i}},\qquad{}
\mathbf o_i=\sum_j p_{\mathrm{prob},i,j}\mathbf v_j$$

其中 $m_{\mathrm{row},i}=\max_j s_{\mathrm{score},i,j}$，$\ell_{\mathrm{exp},i}=\sum_j\exp(s_{\mathrm{score},i,j}-m_{\mathrm{row},i})$。下面假定每个 Query 行至少有一个有效 Key，先说明这一行如何完成计算。

本节末尾的完整程序 `attention_unfused.cu` 实现下面的分离路径，取单 Batch、单 Head、等长 Q/K 序列和因果 Mask。它使用 FP16 Q/K/V，Softmax 后还将 P 显式转为 FP16，分数、累加与输出使用 FP32；参考计算计入相同的类型转换。这个教学实例与后文 BF16 Q、INT8 K/V 的专用融合示例是两套不同配置。
...
对当前 Query 行，保留三个量：已处理分数的最大值 $m_{\mathrm{row},i}$、相对这个最大值计算的指数和 $\ell_{\mathrm{exp},i}$，以及输出分子（尚未归一化的加权和） $\mathbf o_{\mathrm{num},i}$。下标中的 row、exp、num 分别标明行最大值、指数和与输出分子；上标 $(t)$ 表示处理完第 t 个 K/V Tile 后的状态。初始时：

$$m_{\mathrm{row},i}=-\infty,\qquad \ell_{\mathrm{exp},i}=0,\qquad \mathbf o_{\mathrm{num},i}=\mathbf 0$$

设第一个含有效 Key 的 Tile 中，有效位置集合为 $J_1$。直接用这一 Tile 建立状态：

$$m_{\mathrm{row},i}^{(1)}=\max_{j\in J_1}s_{\mathrm{score},i,j},\qquad{}
p_{\mathrm{unnorm},i,j}^{(1)}=\exp(s_{\mathrm{score},i,j}-m_{\mathrm{row},i}^{(1)})$$

$$\ell_{\mathrm{exp},i}^{(1)}=\sum_{j\in J_1}p_{\mathrm{unnorm},i,j}^{(1)},\qquad{}
\mathbf o_{\mathrm{num},i}^{(1)}=\sum_{j\in J_1}p_{\mathrm{unnorm},i,j}^{(1)}\mathbf v_j$$

$p_{\mathrm{unnorm}}$ 是尚未归一化的权重，已经可以和当前 V Tile 相乘。此时保存 $\ell_{\mathrm{exp},i}$，就能把最终的除法延后到所有 K/V Tile 完成之后。

### 6.4.3 新的行最大值怎样修正旧结果

处理下一 Tile 时，新分数可能提高整行的最大值。设第 t 个 Tile 的有效位置集合为 $J_t$，则：

$$m_{\mathrm{row},i}^{(t)}=\max\left(m_{\mathrm{row},i}^{(t-1)},\max_{j\in J_t}s_{\mathrm{score},i,j}\right),\qquad{}
r_{\mathrm{rescale},i}^{(t)}=\exp\left(m_{\mathrm{row},i}^{(t-1)}-m_{\mathrm{row},i}^{(t)}\right)$$

旧的指数和与部分输出都相对于旧最大值计算，因此要同时乘以 $r_{\mathrm{rescale},i}^{(t)}$，再加入当前 Tile 的贡献：

$$\ell_{\mathrm{exp},i}^{(t)}=r_{\mathrm{rescale},i}^{(t)}\ell_{\mathrm{exp},i}^{(t-1)}
+\sum_{j\in J_t}\exp(s_{\mathrm{score},i,j}-m_{\mathrm{row},i}^{(t)})$$

$$\mathbf o_{\mathrm{num},i}^{(t)}=r_{\mathrm{rescale},i}^{(t)}\mathbf o_{\mathrm{num},i}^{(t-1)}
+\sum_{j\in J_t}\exp(s_{\mathrm{score},i,j}-m_{\mathrm{row},i}^{(t)})\mathbf v_j$$

设共处理 $T_{\mathrm{tiles}}$ 个 K/V Tile；全部处理完成后，输出 $\mathbf o_i=\mathbf o_{\mathrm{num},i}^{(T_{\mathrm{tiles}})}/\ell_{\mathrm{exp},i}^{(T_{\mathrm{tiles}})}$。前面的逐 Tile 更新会同时缩放旧的指数和与输出分子，再加入当前 Tile 的贡献；Correction 负责旧输出分子的修正。

Mask 决定 $J_t$，所以必须在求最大值和指数之前生效。若某行在当前 Tile 中没有有效 Key，就保持该行状态不变；若整行都没有有效 Key，Softmax 的分母为零，算子需要另行约定输出行为，具体实现也要支持该约定。

### 6.4.4 把行状态放进融合 Kernel

对一个 Query Work Tile，Kernel 保留 Q 和行状态，依次读取 K/V Tile。QKᵀ MMA 生成分数，Softmax 更新行最大值与指数和，并产生当前未归一化权重。Correction 按行最大值的变化缩放旧的输出分子，PV MMA 则把当前权重与 V 的乘积加入输出分子。全部 K/V Tile 处理完后，再用保存的指数和完成归一化并写回 O。

Pipeline 在这些阶段之间传递分数、权重和部分输出，并协调生产者与消费者。普通 GEMM 的 Epilogue 处理当前 Tile 已完成的 Accumulator；融合 Attention 还要把行最大值、指数和与输出分子保留到下一轮 K/V Tile，因此由专用 Kernel 维护这组状态。

### 6.4.5 专用融合 Kernel 的接口与输入
...
K/V 的 Scale 还需要确定共享范围。当前 d256 示例固定 `head_dim=256`，支持 `scale_granularity=128` 或 `256`；其 [Scale 的逻辑形状](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/examples/python/CuTeDSL/cute/blackwell/kernel/attention/mixed_input_fmha/mixed_input_fmha_prefill_d256.py#L1699-L1706)为 `(batch, heads_k, seqlen_k, head_dim / scale_granularity)`。因此，Scale 按 Batch、KV Head、Token 和特征块分别定位；只有同一个特征块内的元素共享该 Scale。

这些 Scale 在软件输入变换中应用。[K/V 反量化步骤](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/examples/python/CuTeDSL/cute/blackwell/kernel/attention/mixed_input_fmha/prefill_helpers.py#L234-L281)先读取 INT8 数据和对应 Scale，将 INT8 值转换为与 Q 相同的 BF16 操作数类型，再乘块级 Scale，得到后续 MMA 使用的操作数。BF16 是变换后操作数的类型，FP32 是 QK/PV 的累加类型。这里的 K/V Scale 由软件变换消费；NVFP4 的 SFA/SFB 则随 Payload 交给硬件 Block-Scaled MMA。`scale_softmax_log2` 和 `scale_output` 则是另外的整体计算系数，分别作用于分数和最终输出。
...
核对融合结果时，参考计算应使用相同的输入值、反量化规则、Softmax 与输出系数、Mask、序列长度和 Head 映射，再比较 O。输出为有限值只是基本检查；归一化和 KV Head 对应关系由使用相同规则的参考计算检验。
...
### 6.4.7 完整程序：分离式 Attention

保存为 `attention_unfused.cu`。这是两次 GEMM 与独立 Mask/Softmax 阶段组成的完整程序，不是前面介绍的在线 Softmax 融合 Kernel。
...
- **Mixed-input** 允许 A/B 使用不同类型或位宽。例如，量化权重可以比激活使用更窄的存储表示，但所选 MMA 仍有固定的操作数格式要求。因此 Mainloop 先加载存储值，再按需要解包、转换类型和调整布局，使它们成为 MMA 可以读取的操作数。若量化表示还包含 Scale 或零点参数，输入变换也要按对应规则使用这些数据。单纯转换类型并不自动完成反量化，入口见 [Mixed-input Builder](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/gemm/collective/builders/sm100_mixed_input_umma_builder.inl)。
  ...
  “零点”需要按具体接口的计算关系解释。设 q 是量化值，$s_{\mathrm{quant}}$ 是 Scale；常见表示用整数零点 z 写成 $s_{\mathrm{quant}}(q-z)$。本文链接的 SM100 Mixed-input 实现则先转换输入类型、乘 Scale，再加上 `ptr_Z` 提供的值，忽略中间舍入时可写为 $q s_{\mathrm{quant}}+t_{\mathrm{offset}}$，其中 $t_{\mathrm{offset}}$ 是加法项。因此从前一种表示准备参数时，应按 $t_{\mathrm{offset}}=-s_{\mathrm{quant}}z$ 换算，不能直接把 z 传作 $t_{\mathrm{offset}}$。[实际输入变换](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/detail/collective/mixed_input_utils.hpp#L1024-L1083)规定了这一顺序，实际数值还受转换与计算类型影响。这是该实现的参数约定，不是所有零点接口的统一定义；第六部分的 NVFP4 表示不包含这一平移项。

- **软件 Blockwise Scaling** 同样用量化值与 Scale 表示输入，但 Scale 参与运算的位置不同。固定一个输出位置，在两侧 Scale 都不变的 K 区间内，可以先用普通 Tensor MMA 求量化值的部分积，再乘对应的 Scale 乘积，加入完整累加结果。进入下一个缩放区间后，先前的部分积已经按自己的 Scale 合并，新区间则使用新的 Scale。缩放因子随 K 区间变化，因此部分积要先按各自的因子缩放，再合并为完整 K 归约结果。
  ...
  官方示例中的 **Groupwise** 同样讨论矩阵内部的缩放分组，而 **Grouped GEMM** 讨论一次调用中的独立矩阵问题，二者可以组合。例如，Example 81 的 [Groupwise 配置](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/examples/81_blackwell_gemm_blockwise/81_blackwell_gemm_groupwise.cu#L103-L106)取三个粒度为 `(1,128,128)`，使 A 的每一行沿 K 每 128 个元素共享一个 Scale，B 则按 N 与 K 方向的 128×128 块共享。这组尺寸描述该实例的分组方式；其他 Groupwise 配置的共享范围仍由各自的粒度参数确定。

  Blockwise 的 Scale 按矩阵内部的块共享。调用者先准备量化值与逐块 Scale，Collective 再显式执行部分积的缩放与合并，这就是这里“软件”的含义。原始输入的量化由调用者完成，矩阵乘法由 Tensor Core 执行，该路径使用 `OpClassTensorOp`；第六部分的硬件 Block-Scaled 路径则让 MMA 直接消费量化值与 Scale。两条路径分别采用自己的 Scale 粒度、Layout 和参数结构，切换时应按所选实现重新准备输入，入口见 [Blockwise Builder](https://github.com/NVIDIA/cutlass/blob/8f50b052e1099fb982392a622caab69b97b63128/include/cutlass/gemm/collective/builders/sm100_blockwise_umma_builder.inl)。
...
- **Fast-FP32 / 9xBF16** 的输入仍按 FP32 保存，Mainloop 则用多次 BF16 MMA 近似完成乘法。只把 FP32 转成一个 BF16 值，会舍去一部分有效信息；这条路径在取得第一个 BF16 分量后，用原值减去该分量，将残差缩放后继续转换，从而生成能补充前一分量的后续分量。
  ...
  分量保留方式、参与计算的乘积项和累加顺序共同影响误差与计算量。这组 BF16 MMA 与逐项 FP32 FMA 采用不同的舍入和累加过程。Fast 是路径名称，精度与速度仍要用实际输入在目标设备上检验。
...
## 7.4 继续进入 Collective 与 CuTe 的内部实现
