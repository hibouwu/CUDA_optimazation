...
- Its first argument is an instance of [`SM90_TMA_LOAD`](https://github.com/NVIDIA/cutlass/blob/637b15906358191cb4238af419d408a65819d7ec/include/cute/arch/copy_sm90_tma.hpp#L269) . This object dispatches the copy operation to the desired `cp.async.bulk.tensor` PTX call, which we will go into deeper in the third section below.
...
Making a slight modification to the constructor of the TMA store `TiledCopy` object allows us to condense this three-step procedure to just one PTX instruction, namely [`cp.reduce.async.bulk`](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-cp-reduce-async-bulk) instead of `cp.async.bulk` . Precisely, we can make the following one line change on the host code:
...
1. Fire blog post!
Nit: looking at the kernel launches
TMA load launch
<<>>
and TMA store Launch
<<>>
why do you put block_dimension (num_threads_per_cta) before grid_dimension? I thought the standard was to launch <<>>?

  1. it appears the formatting removed the content…apologies
TMA load launch : 1, dim3{M / CTA_M, N / CTA_N, 1}
TMA store launch: CTA_M, dim3{M / CTA_M, N / CTA_N, 1}
standard is: grid_dim, block_dim
but here it does: block_dim, grid_dim
  2. You’re right, thanks for the corrections — now (very belatedly) fixed.
2. Thank you for the amazing post! Question on “In our above example, we supposed that m%CTA_M==0 and n%CTA_N==0. However, for the purposes of doing a TMA load, we can dispense with this assumption entirely.”, what about TMA store?

  1. The same holds for TMA store, it will predicate so you don’t have OOB writes.
