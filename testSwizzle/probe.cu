...
//
// Host 会在未 swizzle 的 global source cells 8、6、7 中分别写入不同的
...
__global__ void print_tma_32b_swizzle_addresses(
    ...
    if (threadIdx.x == 0) {
        ...
        constexpr int kLogicalCells[3] = {8, 6, 7};
        constexpr uint16_t kProbeTags[3] = {0x5108, 0x5106, 0x5107};
        const uint16_t* words = reinterpret_cast<const uint16_t*>(B_storage);
