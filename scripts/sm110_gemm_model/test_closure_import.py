    ...
    def test_report_separates_data_split_from_residency_scenarios(self) -> None:
        ...
        analysis = build_closure_analysis(
            ...
            schedules=[Schedule(
                ...
                causal_pipeline_resource="pipeline.test",
                persistent=True,
            )],
            pipeline_profiles=[PipelineProfile(
                profile_id="profile.test",
                resource="pipeline.test",
                schedule_id="s",
                precision_ids=("bf16_f32",),
                ...
                source_locator="synthetic joint profile",
                input_residency="hot_l2",
                stages=2,
                accumulator_buffers=2,
                resident_ctas_per_sm=1,
                maximum_k_tiles=64,
                maximum_output_tasks_per_worker=64,
                tma_first_completion_seconds=1e-9,
                tma_completion_interval_seconds=1e-9,
                mma_first_completion_seconds=1e-9,
                mma_completion_interval_seconds=1e-9,
                joint_first_mma_completion_seconds=1e-9,
                joint_completion_interval_seconds=1e-9,
                epilogue_latency_seconds=1e-9,
                component_r_squared={"tma": 1.0, "mma": 1.0, "joint": 1.0},
                max_calibration_relative_error=0.0,
                max_holdout_relative_error=0.0,
                fit_contract={
                    "calibration_points": [1],
                    "holdout_points": [64],
                    "calibration_output_tasks": [1],
                    "epilogue_calibration_output_tasks": [1],
                    "holdout_output_tasks": [64],
