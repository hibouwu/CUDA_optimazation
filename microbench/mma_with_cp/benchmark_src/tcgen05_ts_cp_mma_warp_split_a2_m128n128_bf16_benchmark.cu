...
static constexpr long long kMacPerInst = 262144LL;
static constexpr int kInstK = 16;
static constexpr int kBlockThreads = 128;
static constexpr int kWarpsPerBlock = 4;
static constexpr int kIssuerCount = 2;
static constexpr int kInitialBarrierArriveCount = 1;
static constexpr int kCommitArriveCount = 2;
static constexpr int kMmaIssuerCount = 1;
static constexpr int kCpIssuerCount = 1;
static constexpr int kProcessedTilesPerIter = 1;
static constexpr int kCpInstructionsPerTile = 1;
static constexpr int kEffectiveBytesPerCp = 2048;
static constexpr int kMainloopKBlocks = 1;
static constexpr int kMmaAPanelBytes = 4096;
static constexpr int kMmaBPanelBytes = 4096;
static constexpr int kDescLeading = 16;
static constexpr int kDescStride = 8;
static constexpr char kCaseId[] = "ts_cp_mma_warp_split_a2";
static constexpr char kCaseLabel[] = "TS CP+MMA Warp Split A2";
static constexpr char kPrecision[] = "BF16";
static constexpr char kShape[] = "M128N128K16";
static constexpr char kCpSuffix[] = "128x128b";
...
__device__ __forceinline__ void issue_cp(uint32_t taddr, uint64_t s_desc) {
  asm volatile("tcgen05.cp.cta_group::1.128x128b [%0], %1;" :: "r"(taddr), "l"(s_desc) : "memory");
}
...
void tcgen05_kernel(int iters, unsigned long long* cycles_out) {
  ...
  uint64_t a_desc = make_smem_desc(smem_a, 16, 8);
  uint64_t b_desc = make_smem_desc(smem_b, 16, 8);
  uint64_t idesc = make_idesc_bf16();
  uint32_t d_tmem = tmem_base;
  uint32_t a_tmem0 = tmem_base + 256;
  uint32_t a_tmem1 = tmem_base + 320;
  uint32_t tsfa = 0; uint32_t tsfb = 0;
...
int main(int argc, char** argv) {
  ...
  long long mma_instruction_count = static_cast<long long>(active_blocks) * static_cast<long long>(kMmaIssuerCount) * static_cast<long long>(iters);
  long long cp_instruction_count = static_cast<long long>(active_blocks) * static_cast<long long>(kCpIssuerCount) * static_cast<long long>(iters);
  long long processed_tiles = static_cast<long long>(active_blocks) * static_cast<long long>(kProcessedTilesPerIter) * static_cast<long long>(iters);
