...
static constexpr int kInstK = 32;
...
static constexpr int kDescLeading = 8;
static constexpr int kDescStride = 4;
...
static constexpr char kShape[] = "M128N128K32";
...
void tcgen05_kernel(int iters, unsigned long long* cycles_out) {
  ...
  uint64_t a_desc = make_smem_desc(smem_a, 8, 4);
  uint64_t b_desc = make_smem_desc(smem_b, 8, 4);
